...
def docker_check():
    try:
        ...
        # If the output contains "Docker version", then Docker is installed
        if "Docker version" in output.decode('utf-8'):
            print("Docker is installed on this machine.")
        else:
            # If Docker is not installed, run the `curl` command to install it
            subprocess.run(['curl', '-fsSL', 'https://get.docker.com', '|', 'sh'], check=True)
            print("Docker has been installed on this machine.")
    except subprocess.CalledProcessError:
        # If the `docker` command fails, run the `curl` command to install Docker
        subprocess.run(['curl', '-fsSL', 'https://get.docker.com', '|', 'sh'], check=True)
        print("Docker has been installed on this machine.")
docker_check()
