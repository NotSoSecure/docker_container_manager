...
def create_container(ssh_key_filename, server_ip_address):
    ...
    remote_script_path = f"{remote_script_dir}/server.py"

    clone_repository(ssh_command, remote_script_dir)

    # Check if Docker image is already present
    image_name = "notsosecure/rsh_kali:latest"
    check_image_command = f"{ssh_command} sudo docker images --format '{{.Repository}}:{{.Tag}}' | grep {image_name}"
    check_image_output = subprocess.run(check_image_command, shell=True, capture_output=True, text=True)
    if check_image_output.returncode != 0:
        # Download Docker image
        download_image_command = f"{ssh_command} sudo docker pull {image_name}"
        logging.info(f"Downloading Docker image: {image_name}")
        subprocess.run(download_image_command, shell=True, check=True)

    change_dir_command = f"{ssh_command} cd {remote_script_dir} && pwd"
    remote_script_dir_output = subprocess.run(change_dir_command, shell=True, check=True, capture_output=True, text=True).stdout.strip()
    logging.info(f"Changed directory to {remote_script_dir_output}")
    ...
    remote_file_path = f"/home/ubuntu/{name}_{expiry}.zip"
...
def stop_container(ssh_key_filename, server_ip_address):
    ...
    if stop_containers.lower() in ["yes", "y"]:
        containers = input(
